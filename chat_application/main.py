...
async def message(sid, data):
    ...
    print("message", user, conversation, data)
...
async def follow_up(sid, title):
    ...
    print("conversation_json", conversation_json, type(conversation_json))
    messages = conversation_json[0].get("messages", [])
    print("messages", messages, type(messages))
    response= get_follow_up_question(messages)
